...
package sus_sli_etl.record_count_compare_0_1;
...
/**
 * Job: Record_Count_Compare Purpose: Verification of migrated data from SUS to SLI<br>
 * Description: Verification of migrated data from SUS to SLI <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class Record_Count_Compare implements TalendJob {
...
public String getSUS_Additional_Params(){
	...
	private final String jobName = "Record_Count_Compare";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Record_Count_Compare.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Record_Count_Compare.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class Main_StatStruct implements routines.system.IPersistableRow<Main_StatStruct> {
    final static byte[] commonByteArrayLock_SUS_SLI_ETL_Record_Count_Compare = new byte[0];
    static byte[] commonByteArray_SUS_SLI_ETL_Record_Count_Compare = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_SUS_SLI_ETL_Record_Count_Compare.length) {
				if(length < 1024 && commonByteArray_SUS_SLI_ETL_Record_Count_Compare.length == 0) {
   					commonByteArray_SUS_SLI_ETL_Record_Count_Compare = new byte[1024];
				} else {
   					commonByteArray_SUS_SLI_ETL_Record_Count_Compare = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_SUS_SLI_ETL_Record_Count_Compare, 0, length);
			strReturn = new String(commonByteArray_SUS_SLI_ETL_Record_Count_Compare, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_SUS_SLI_ETL_Record_Count_Compare, 0, length);
			strReturn = new String(commonByteArray_SUS_SLI_ETL_Record_Count_Compare, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_SUS_SLI_ETL_Record_Count_Compare) {
 ...
 */
    ...
    public static void main(String[] args){
        final Record_Count_Compare Record_Count_CompareClass = new Record_Count_Compare();

        int exitCode = Record_Count_CompareClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Record_Count_Compare.class.getClassLoader().getResourceAsStream("sus_sli_etl/record_count_compare_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Record_Count_Compare.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Record_Count_Compare");
        }
